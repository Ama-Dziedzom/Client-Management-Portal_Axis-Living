...
export default function EditJournalPostPage() {
    ...
    const [form, setForm] = useState<Pick<WebsitePost, 'title' | 'slug' | 'body' | 'cover_image' | 'published' | 'published_at'>>({
        title: '', slug: '', body: null, cover_image: null, published: false, published_at: null,
    })

    useEffect(() => {
        supabase.from('website_posts').select('*').eq('id', id).single().then(({ data }) => {
            if (data) setForm({ title: data.title, slug: data.slug, body: data.body, cover_image: data.cover_image, published: data.published, published_at: data.published_at })
            setLoading(false)
        })
    }, [id])
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setSaving(true)
        setError('')
        const publishedAt = form.published && !form.published_at ? new Date().toISOString() : form.published_at
        const { error: err } = await supabase.from('website_posts').update({
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h1 className="text-3xl font-heading font-semibold text-text-primary">Edit Post</h1>
            ...
            <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
                ...
                <div className="card-flat space-y-5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-text-secondary mt-1">URL: /journal/{form.slug}</p>
                    </div>
                    <div>
                ...
                <div className="card-flat">
                    ...
                    <textarea value={form.body ?? ''} onChange={e => set('body', e.target.value)} rows={16} className="input-field resize-y font-mono text-sm" />
                ...
                <div className="card-flat">
                    <label className="flex items-center gap-3 cursor-pointer">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-secondary">Visible on the website</p>
                        </div>
                    </label>
                </div>
